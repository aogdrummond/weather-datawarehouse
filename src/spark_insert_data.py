...
def parse_locations_df(df:DataFrame)->DataFrame:
    ...
    """
    raw_locations_df = df.selectExpr('city','country','region','lat','long','tz_id')
    trans_locations_df = transform_data_in_locations(raw_locations_df)
    verify_location_udf = udf(verify_location,IntegerType())
    locations_df_with_id = trans_locations_df.withColumn('id',verify_location_udf(trans_locations_df['city']\
                                                                    ,trans_locations_df['lat']\
                                                                    ,trans_locations_df['long']))
    identified_location_df = locations_df_with_id.selectExpr('*','id is not NULL as is_identified')
    new_locations_df = identified_location_df.filter(col('is_identified')==False)
    insert_locations_df = new_locations_df.drop('id','is_identified').withColumn('created_at',current_timestamp())
...
def verify_location(city: str, lat: Union[str, None] = None, long: Union[str, None] = None) -> int:
    """
...
def parse_weathers_df(df:DataFrame)->DataFrame:
    ...
    verify_location_udf = udf(verify_location,IntegerType())
    weathers_df_with_id = trans_weathers_df.withColumn('location_id',verify_location_udf(trans_weathers_df['city']))      
    inserted_weathers_df = weathers_df_with_id.drop('city').withColumn('created_at',current_timestamp())
